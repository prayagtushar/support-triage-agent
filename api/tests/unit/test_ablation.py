"""The ablation is only as trustworthy as signals_of, which these tests pin in CI."""
...
def test_failed_classification_reconstructs_as_none():
    """A ticket that never classified must not come back as a confident one."""
...
def test_arm_settings_uses_report_policy_not_current_config():
    """Replaying an old report under today's thresholds is the bug this guards."""
...
def test_no_judge_arm_zeroes_the_judge_contribution():
    ...
    # Perfect judge, imperfect retrieval: dropping the judge must lower the composite.
...
def test_hard_rules_are_identical_across_arms():
    """Hard rules fire before the composite, so no reweighting can move those tickets."""
    for signals in (
